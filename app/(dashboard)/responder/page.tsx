...
export default function ResponderPage() {
    ...
    return (
        <div className="min-h-[calc(100vh-4rem)] bg-slate-50 flex flex-col font-sans">
            ...
            <header className="bg-white border-b border-slate-200 p-4 shadow-sm z-30 sticky top-0">
                <div className="container max-w-md mx-auto">
                    <div className={`p-4 rounded-xl border-2 transition-all flex items-center justify-between ${status === 'available' ? 'bg-green-50 border-green-200' :
                        status === 'busy' ? 'bg-blue-50 border-blue-200' :
            ...
            <main className="flex-1 container max-w-md mx-auto p-4 flex flex-col relative">
                ...
                <AnimatePresence>
                    {request && !activeCase && (
                        <motion.div
                            ...
                            animate={{ y: 0 }}
                            exit={{ y: "100%" }}
                            className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-end sm:items-center sm:justify-center"
                        >
                            <div className="w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden p-6 pb-8 space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-2xl font-bold text-slate-900">{request.type}</h3>
                                            <p className="text-slate-500">{request.location} • {request.distance}</p>
                                        </div>
                                    </div>

                                    <Alert className="bg-blue-50 border-blue-100">
                                        <Shield className="h-4 w-4 text-blue-600" />
                                        <AlertTitle className="text-blue-800">AI Triage Note</AlertTitle>
                                ...
                                <div className="grid gap-3">
                                    <Button onClick={handleAccept} className="w-full h-14 text-lg font-bold bg-green-600 hover:bg-green-700 shadow-lg shadow-green-200">
                                        Accept Request • {request.eta}
                                    </Button>
                                    <Button onClick={handleDecline} variant="ghost" className="w-full text-slate-500 hover:bg-slate-100 hover:text-slate-900">
                ...
                {activeCase && !showResolution && (
                    <div className="space-y-4 flex-1 flex flex-col">
                        {/* Map Panel */}
                        <div className="h-64 bg-slate-200 rounded-2xl overflow-hidden border border-slate-300 relative shadow-inner">
                            <Map className="h-full w-full" />
                            <div className="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold shadow-sm flex items-center gap-2">
                        ...
                        <Card className="border-l-4 border-l-blue-500 shadow-sm">
                            <CardContent className="p-4 space-y-4">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h3 className="font-bold text-lg text-slate-900 flex items-center gap-2">
                                            {activeCase.user} <span className="text-xs font-normal text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">User</span>
                                        </h3>
                                        <p className="text-slate-600 text-sm mt-1">{activeCase.location}</p>
                                    </div>
                                ...
                                <div className="flex gap-2">
                                    <Button className="flex-1 bg-blue-600 hover:bg-blue-700 text-white shadow-blue-100">
                                        <Navigation className="h-4 w-4 mr-2" /> Navigate
                                    </Button>
                                    <Button variant="outline" className="flex-1 border-slate-200 text-slate-700">
                                        <Phone className="h-4 w-4 mr-2" /> Call
                                    </Button>
                                    <Button variant="outline" size="icon" className="border-slate-200 text-slate-700">
                                ...
                                <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                                    {["I'm on my way", "Stay on the line", "I'm here"].map(msg => (
                                        <button key={msg} className="text-nowrap px-3 py-1.5 bg-slate-50 text-slate-600 text-xs font-medium rounded-full border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-colors">
                                            {msg}
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className="mt-auto grid grid-cols-2 gap-3 pb-safe">
                            ...
                            <Button variant="secondary" onClick={handleComplete} className="w-full bg-slate-900 text-white hover:bg-slate-800 shadow-lg shadow-slate-200">
                ...
                {showResolution && (
                    ...
                    >
                        <div className="text-center space-y-2">
                            ...
                            <h2 className="text-2xl font-bold text-slate-900">Initial Response Complete</h2>
                            <p className="text-slate-500">Great work. Take a moment to breathe.</p>
                        </div>

                        <Card>
                            <CardHeader>
                                <CardTitle className="text-base">Incident Outcome</CardTitle>
                            </CardHeader>
                            <CardContent className="grid gap-3">
                                <Button variant="outline" className="justify-start h-12">
                                    <CheckCircle className="h-4 w-4 mr-2 text-green-500" /> Resolved On-Site
                                </Button>
                                <Button variant="outline" className="justify-start h-12">
                                    <Phone className="h-4 w-4 mr-2 text-red-500" /> Handed over to Ambulance
                                </Button>
                                <Button variant="outline" className="justify-start h-12">
                                    <XCircle className="h-4 w-4 mr-2 text-slate-400" /> User Unavailable / False Alarm
                                </Button>
                            </CardContent>
                        </Card>

                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-center">
                            <p className="text-blue-800 font-bold mb-1">Emotional Check-in</p>
                            <p className="text-xs text-blue-600 mb-3">That was a high-stress situation. Are you okay to continue?</p>
                            <div className="flex gap-3 justify-center">
                                <Button onClick={finalizeResolution} size="sm" className="bg-blue-600 text-white">I'm Good</Button>
                                <Button onClick={() => { finalizeResolution(); setStatus('offline') }} size="sm" variant="outline" className="border-blue-200 text-blue-700 bg-white">Go Offline</Button>
                            </div>
                        </div>
                    </motion.div>
                )}
                ...
                {!status.includes('busy') && !request && !activeCase && !showResolution && (
                    <div className="flex-1 flex flex-col items-center justify-center text-center text-slate-400 p-8 opacity-50">
                        <div className="h-24 w-24 rounded-full border-4 border-slate-200 flex items-center justify-center mb-4">
                        ...
                        {/* Demo Helper Text */}
                        <p className="absolute bottom-4 text-[10px] text-slate-300 uppercase tracking-widest w-full text-center">
